...
static const char *const luaX_tokens [] = {
    ...
    "default", "each", "extends", "finally", "implements", "import", 
    "interface", "new", "override", "private", "public", "protected", "static",
...
*/
static int jual_count_doublequotes (LexState *ls) {
  int count = 1;
  lua_assert(ls->current == '"');
  save_and_next(ls);
...
static void jual_read_long_string (LexState *ls, SemInfo *seminfo, int sep) {
...
//  save_and_next(ls);  /* skip 2nd '[' */
  ...
  for (;;) {
    switch (ls->current) {
      ...
      case '"': {
        if (jual_count_doublequotes(ls) == 3) goto endloop;
        break;
      }      
/*
      case ']': {
...
static void jual_read_string (LexState *ls, int del, SemInfo *seminfo) {
  ...
  for (;;) {
    switch (ls->current) {
      ...
      case '"': {  /* multiline string or string */
        int sep = jual_count_doublequotes(ls);
        if (sep == 3) {
          jual_read_long_string(ls, seminfo, 1);
        }
        else if (sep == 2) {
          // empty string
          seminfo->ts = luaX_newstring(ls, luaZ_buffer(ls->buff) + 1,
