...
tcp_socket *TCPClient(tcp_socket *ts, const char *address, int port, int timeout) {
    int initState;
    if (!WSAInitilized) initState = WSAInit();
    ...
    if (connect(ts->sock, (SOCKADDR *)&sin, sizeof(sin)) == SOCKET_ERROR) {
        return NULL;
        WSACleanup();
    };
...
tcp_socket *TCPServer(tcp_socket *ts, const char *address, int port, int timeout, void *onConnection) {
    ...
    if (bind(ts->sock, (SOCKADDR *) &sin, sizeof(sin)) == SOCKET_ERROR) {
    ...
    if (listen(ts->sock, MAX_CONNECTIONS) == SOCKET_ERROR) {
...
tcp_socket *TCPAccept(tcp_socket *ts, tcp_socket *tc) {
    ...
    SOCKET ClientSocket = accept(ts->sock, (struct sockaddr *)&client_info, &size);
